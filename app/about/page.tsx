...
const peopleData = [
  { id:  1, name: "Aiko Tanaka",        role: "",                photo: null },
  { id:  2, name: "Anjana Ramesh",      role: "",                photo: null },
  { id:  3, name: "Anna Phaidra",       role: "Visual Design",   photo: null },
  { id:  4, name: "Barra MacMahon",     role: "",                photo: null },
  { id:  5, name: "Caleb Osei",         role: "",                photo: null },
  { id:  6, name: "Chenlu Ni",          role: "",                photo: null },
  { id:  7, name: "Diego Herrera",      role: "",                photo: null },
  { id:  8, name: "Elena Vasquez",      role: "",                photo: null },
  { id:  9, name: "Fatima Al-Rashid",   role: "",                photo: null },
  { id: 10, name: "Gabrielle Francois", role: "",                photo: null },
  { id: 11, name: "Henrik Larsson",     role: "",                photo: null },
  { id: 12, name: "Iris Chen",          role: "",                photo: null },
  { id: 13, name: "Javiera Bilbao",     role: "Project Manager", photo: null },
  { id: 14, name: "Lucas Mendes",       role: "",                photo: null },
  { id: 15, name: "Maya Patel",         role: "",                photo: null },
  { id: 16, name: "Nadia Kowalski",     role: "",                photo: null },
]
...
export default function AboutPage() {
  ...
  const lineHeight = 80
  const padding = 10
  const lineHeightPct = (lineHeight / panelHeight) * 100
  ...
  return (
    <div className="pt-9 min-h-screen bg-[#fcfaf2] text-[#222] selection:bg-[#f0f0f0] font-sans overflow-x-hidden -mx-6 sm:-mx-12 md:-mx-16 lg:-mx-24 xl:-mx-32">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-8 md:px-16 pt-1 pb-4">
        ...
        <div className="w-full md:w-1/2 border-2 border-black flex overflow-hidden" style={{ maxHeight: "420px" }}>
          {/* Scrollable list */}
          <div ref={listRef} className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
            {peopleData.map((person, i) => (
              <div key={person.id} className={i > 0 ? "border-t-2 border-black" : ""}>
                <button
                  className="w-full flex items-center justify-between px-6 py-4 text-left hover:bg-[#f0efe7] transition-colors duration-200"
                ...
                >
                  <div className="overflow-hidden">
                    <div className="border-t border-black bg-[#FBFAF1]">
                      {person.photo ? (
                        <img src={person.photo} alt={person.name} className="w-full object-cover" />
                      ) : (
                        <div className="w-full aspect-[4/3] bg-[#D5D4CD]/40 flex items-center justify-center">
          ...
          {/* Scroll indicator */}
          <div className="w-4 border-l-2 border-black flex-shrink-0 relative bg-[#FBFAF1]">
            <div
              className="absolute w-1 h-20 bg-black rounded-full transition-all duration-100"
              style={{ left: "50%", top: `${lineTop}%`, transform: "translateX(-50%)" }}
            />
          </div>
        </div>
      </div>
